...
import { usePendingUsers, useAllUsers, useApproveUser, useDenyUser, useUpdateUserStatus, useRemoveUser, PendingUser, AllUser } from '@/lib/queries/auth'
...
export default function ApprovalsScreen() {
  ...
  // Redirect non-admins away (with safety check to prevent navigation before router is ready)
  useEffect(() => {
    // Only redirect if profile has finished loading and user is not admin
  ...
  if (profileLoading || (activeTab === 'pending' ? pendingLoading : allLoading)) {
    return (
  ...
  const handleApprove = (user: PendingUser | AllUser) => {
    console.log('Approve button clicked for user:', user)
  ...
  const confirmApprove = async () => {
    ...
    console.log('Confirming approval:', { userId: selectedUser.id, makeAdmin })
          ...
          {showEdit ? (
            ...
            <>
              <TouchableOpacity
                ...
                onPress={(e) => {
                  e.stopPropagation()
                  console.log('Approve button pressed in UserCard')
                  onApprove(user)
                }}
                disabled={isLoading}
                activeOpacity={0.8}
              ...
              <TouchableOpacity
                ...
                onPress={(e) => {
                  e.stopPropagation()
                  console.log('Deny button pressed in UserCard')
                  onDeny()
                }}
                disabled={isLoading}
                activeOpacity={0.8}
